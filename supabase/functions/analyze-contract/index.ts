...
serve(async (req) => {
  ...
  // Create a service-role client for guaranteed writes
...
- auto_renewal: SET TO TRUE if you found ANY auto-renewal clause. Only set false if the contract explicitly states no auto-renewal.
...
IMPORTANT: The clauses array is the source of truth. Metadata fields must be consistent with extracted clauses. Never return null/N/A if a relevant clause exists. Return only valid JSON.`;
    ...
    // Call AI with a 25-second timeout
    ...
    await serviceClient.from("contract_clauses").delete().eq("contract_id", contract_id);
    ...
    if (analysis.clauses?.length) {
      ...
      await serviceClient.from("contract_clauses").insert(clauseRows);
    }
    ...
    // Derive auto_renewal from clauses
